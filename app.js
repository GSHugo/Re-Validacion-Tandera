...
$('#scanForm').onsubmit=async e=>{e.preventDefault();const i=$('#code'),codigo=i.value.trim();if(!codigo)return;try{state.pallet=await api('scan',{idTarima:state.pallet.pallet.ID_TARIMA,codigo,operador:operator()});i.value='';$('#lastScan').textContent='✓ '+codigo.toUpperCase()+' guardado';renderScan();i.focus();navigator.vibrate?.(80)}catch(err){toast(err.message)}};
