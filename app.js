...
const state={pallet:null,locations:[],stream:null,detector:null,detecting:false};
const cfg=window.TANDERA_CONFIG||{};
async function api(action,data={}){if(!cfg.API_URL||cfg.API_URL.includes('PEGAR_AQUI'))throw new Error('Configura API_URL en config.js.');loading(true);try{const r=await fetch(cfg.API_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({...data,action,apiKey:cfg.API_KEY||''}),redirect:'follow'}),j=await r.json();if(!j.ok)throw new Error(j.error||'Error del servidor');return j.data}finally{loading(false)}}
...
$('#scanForm').onsubmit=async e=>{e.preventDefault();const i=$('#code'),codigo=i.value.trim();if(!codigo)return;try{const saved=await api('scan',{idTarima:state.pallet.pallet.ID_TARIMA,codigo,operador:operator()});state.pallet.total=saved.total;state.pallet.pallet.TOTAL_BULTOS=saved.total;const row=state.pallet.summary.find(x=>x.codigo===saved.codigo);if(row)row.cantidad++;else state.pallet.summary.push({codigo:saved.codigo,cantidad:1});state.pallet.summary.sort((a,b)=>a.codigo.localeCompare(b.codigo));i.value='';$('#lastScan').textContent='✓ '+saved.codigo+' guardado · '+saved.total+' bultos';renderScan();i.focus();navigator.vibrate?.(80)}catch(err){toast(err.message)}};
...
$('#closeBtn').onclick=async()=>{const id=$('#shortLocation').textContent;try{state.pallet=await api('assignLocation',{idTarima:state.pallet.pallet.ID_TARIMA,idUbicacion:id});localStorage.removeItem('tandera_open');toast('Tarima cerrada en '+id);await refreshPallets();renderDetail(state.pallet);view('detail')}catch(e){toast(e.message)}};
